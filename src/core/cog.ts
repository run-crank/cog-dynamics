    ...
    if (this.redisUrl) {
      ...
      c.on('error', () => {
        ...
        if (this.mailgunCredentials.apiKey && this.mailgunCredentials.domain && this.mailgunCredentials.alertEmail && !emailSent) {
          ...
          const emailData = {
            from: `Dynamics Cog <noreply@${this.mailgunCredentials.domain}>`,
            to: this.mailgunCredentials.alertEmail,
            subject: 'Broken Redis Url in Dynamics Cog',
            text: 'The redis url in the Dynamics Cog is no longer working. Caching is disabled for the Dynamics Cog.',
          };
