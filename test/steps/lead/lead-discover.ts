...
describe('DiscoverLeadStep', () => {
  ...
  it('should return expected step metadata', () => {
    ...
    expect(stepDef.getName()).to.equal('Discover fields on a Dynamics CRM Lead');
    expect(stepDef.getExpression()).to.equal('discover fields on dynamics crm lead (?<email>.+)');
  ...
  it('should respond with pass if lead is found.', async () => {
  ...
  it('should respond with fail if lead is not found.', async () => {
    // A request with firstname as sample field
    const retrieveRequest = {
      collection: 'leads',
